    ...
    <div
      className={`relative overflow-hidden shrink-0 ${
        compact
          ...
          : tall
            ? "h-52 md:h-full md:min-h-[240px] md:w-[46%]"
            : "h-44 w-full"
      }`}
    >
      <img
...
const ServiceCard = memo(
  ({ title, description, iconKey, imageUrl, features = [], index, featured = false }) => {
          ...
          >
            <div className="h-full flex flex-col md:flex-row overflow-hidden rounded-2xl bg-tertiary/90 border border-white/10 transition-all duration-500 hover:border-[#915EFF]/45 hover:shadow-[0_20px_60px_rgba(145,94,255,0.18)]">
              ...
              <div className="relative flex flex-col justify-center flex-1 p-6 sm:p-8">
                ...
                {description && (
                  <p className="mt-3 text-secondary text-sm sm:text-[15px] leading-relaxed">{description}</p>
                )}
    ...
    return (
      <motion.article variants={fadeIn("up", "spring", index * 0.05, 0.55)} className="group relative">
        <a
          href="#contact"
        ...
        <div className="hidden md:flex h-full flex-col overflow-hidden rounded-2xl bg-tertiary/90 border border-white/10 transition-all duration-500 hover:border-[#915EFF]/40 hover:shadow-[0_16px_48px_rgba(145,94,255,0.14)] hover:-translate-y-1">
